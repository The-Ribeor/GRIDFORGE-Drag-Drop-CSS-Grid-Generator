...
export const HelpModal = ({ onClose, lang }: { onClose: () => void, lang: Language }) => {
  ...
  return (
    <div 
      className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md transition-all" 
      onClick={onClose}
    >
      <div 
        className="bg-card-bg border border-border-main w-full max-w-2xl max-h-[90vh] rounded-[2rem] shadow-[0_0_50px_-12px_rgba(0,0,0,0.5)] flex flex-col animate-in fade-in zoom-in duration-300 relative overflow-hidden" 
        onClick={e => e.stopPropagation()}
      >
        {/* Decoración de fondo */}
        <div className="absolute -top-24 -right-24 w-48 h-48 bg-blue-500/10 blur-[80px] pointer-events-none" />
        ...
        {/* HEADER: Fijo arriba */}
        <div className="flex justify-between items-start p-6 md:p-10 pb-2 md:pb-4 relative z-10">
          <div className="space-y-2">
            <span className="font-black text-blue-500 uppercase tracking-[0.3em] text-[9px] md:text-[10px] bg-blue-500/10 px-3 py-1 rounded-full border border-blue-500/20">
              {t.guide}
            </span>
            <h2 className="text-text-title font-black text-xl md:text-2xl leading-tight tracking-tighter uppercase italic transition-colors">
              {t.title}
            </h2>
          </div>
          <button 
            onClick={onClose} 
            className="text-text-body hover:text-text-title hover:bg-app-bg rounded-full p-2 transition-all shrink-0"
          >
            <X size={24} />
          </button>
        </div>

        {/* CONTENIDO SCROLLABLE: El grid de pasos */}
        <div className="flex-1 overflow-y-auto px-6 md:px-10 py-4 no-scrollbar relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 pb-4">
            {steps.map((step, idx) => (
              ...
              >
                ...
                <p className="text-[10px] md:text-[11px] font-bold leading-relaxed text-text-body group-hover:text-text-title transition-colors">
                  {step.text}
                </p>
              </div>
            ))}
        ...
        {/* FOOTER: Fijo abajo */}
        <div className="p-6 md:p-10 pt-4 border-t border-border-main flex flex-col gap-4 md:gap-5 relative z-10 bg-card-bg/80 backdrop-blur-sm">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 text-[9px] md:text-[10px] font-black uppercase tracking-widest text-slate-500 px-1">
            <span className="opacity-50 tracking-[0.2em]">Engine v1.0.0</span>
            <div className="flex items-center gap-1.5">
              <span>{t.author}</span>
              <a 
                ...
                className="text-text-title hover:text-blue-500 transition-colors underline decoration-blue-500/30 underline-offset-4"
              >
                @DiegoBerrio1601
              </a>
              <Heart size={10} className="fill-red-500 text-red-500 animate-pulse" />
          ...
          <button 
            onClick={onClose} 
            className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-4 md:py-5 rounded-2xl uppercase text-[11px] md:text-[12px] tracking-[0.3em] md:tracking-[0.4em] transition-all shadow-[0_10px_20px_-10px_rgba(59,130,246,0.5)] active:scale-[0.97]"
          >
            {t.button}
          </button>
        </div>
