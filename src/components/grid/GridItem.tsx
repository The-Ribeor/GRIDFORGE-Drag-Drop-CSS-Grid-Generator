...
// import { GridElement } from '@/lib/types';

// export const GridItem = ({ item, onRemove, onResize }: { item: GridElement, onRemove: (id: string) => void, onResize: (id: string) => void }) => {
...
//       ref={setNodeRef}
//       style={style}
//       className={`relative bg-white border-2 border-amber-200 rounded-lg shadow-md flex items-center justify-center text-2xl font-bold select-none ${isDragging ? 'opacity-50' : 'opacity-100'}`}
//     >
//       {/* Zona de arrastre (el número) */}
//       <div {...listeners} {...attributes} className="w-full h-full flex items-center justify-center cursor-grab active:cursor-grabbing">
//         {item.number}
//       </div>

//       {/* Botón Cerrar */}
//       <button onClick={() => onRemove(item.id)} className="absolute top-1 right-1 p-1 bg-red-500 text-white rounded hover:bg-red-600 transition-colors">
//         <X size={12} />
//       </button>

//       {/* Botón Resize */}
//       <button 
//         onMouseDown={(e) => { e.stopPropagation(); onResize(item.id); }}
//         className="absolute bottom-1 right-1 cursor-se-resize text-gray-400 hover:text-black"
//       >
